...
const MONITOR_SETTINGS_KEY = 'sanare-radar-monitor-settings-v2';
...
function updateMonitorStatusNote() {
  if (!els.monitorStatusNote) return;
  const settings = getMonitorSettings();
  els.monitorStatusNote.textContent = settings.serpApiKey
    ? 'Modo actual: snippets automáticos por SERP API activados.'
...
async function runDailyMonitor(silent = false) {
  ...
  try {
    const report = settings.serpApiKey
    ...
    if (!silent) alert(settings.serpApiKey ? 'Reporte diario generado con snippets automáticos.' : 'Reporte diario generado correctamente.');
  } catch (error) {
    console.error(error);
...
function renderDailyReport() {
  ...
  els.reportSummary.innerHTML = `
    ...
    <strong>Modo:</strong> ${report.mode === 'serpapi' ? 'SERP API con snippets' : 'Manual consolidado'}<br>
